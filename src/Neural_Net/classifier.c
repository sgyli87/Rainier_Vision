    ...
    for(i = 0; i < m.rows; ++i){
        for(j = 0; j < m.cols; ++j){
            ...
            if(a == LOGISTIC){
                ...
                d.data[i][j] = ( x <= 0 )? 0:d.data[i][j];
            }
            else if (a == LRELU){
                d.data[i][j] = ( x <= 0 )? 0.1*d.data[i][j]:d.data[i][j];
            }
        }
    }
}
...
matrix backward_layer(layer *l, matrix delta)
{
    ...
    matrix wT = transpose_matrix(l->w); // replace this
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    // save it to l->v
    matrix q1 = axpy_matrix(momentum,l->v, l->dw);
    l->v = axpy_matrix(-decay,l->w,q1);
    free_matrix(q1);
