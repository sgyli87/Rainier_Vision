    ...
    for(int i = 0; i < im.w; i++) {
        for(int j = 0; j < im.h; j++) {
            for(int k = 0; k < im.c; k++) {
                ...
                if(i > 0) val += get_pixel(integ, i - 1, j, k);
                if(j > 0) val += get_pixel(integ, i, j - 1, k);
                if(i > 0 && j > 0) val -= get_pixel(integ, i - 1, j - 1, k);
                set_pixel(integ, i, j, k, val);
            }
        }
    }
    return integ;
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            if (M_In.data){
                vx = M_In.data[0][0] * -Ixt + M_In.data[0][1] * -Iyt;
                vy = M_In.data[1][0] * -Ixt + M_In.data[1][1] * -Iyt;
            }
